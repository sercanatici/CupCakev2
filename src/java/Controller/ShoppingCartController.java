...
public class ShoppingCartController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if(user.getBalance() - price < 0){
            request.getRequestDispatcher("broke.jsp").forward(request, response);
        } else {
        ...
        orderList.add(new Order(bottom, topping, price));
        int newPrice = user.getBalance() - price;
        
        user = da.updateBalance(user, newPrice);
        sesh.setAttribute("user", user);
        
        if(!orderList.isEmpty()){
            sesh.setAttribute("orderlist", orderList);
